            ...
            if len(line) != 0:
                ...
                if lsplit[1] == "WORDTAG":
                    if Countword[lsplit[3]] < 5:
                        lsplit[3] = "_RARE_"
                    if emmision.get(Element(lsplit[2],lsplit[3])):
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if len(line) != 0:
                lsplit = line.split(" ")
                if lsplit[1] == "3-GRAM":
                    if Countgram.get(twogramElement(lsplit[2],lsplit[3])):
...
def viterbi(words):
        ...
        for u in range(numtags):
            for v in range(numtags):
                ...
                for w in range(numtags):
                    temp = 0
                    if threegram.get(threegramElement(tags[w],tags[u],tags[v])) and emmision.get(Element(tags[v],word[k-1])):
    ...
    for u in range(numtags):
        for v in range(numtags):
            if threegram.get(threegramElement(tags[u],tags[v],'STOP')):
...
Element = namedtuple("Element", ["tag", "word"])
threegramElement = namedtuple("threegramElement", ["first", "second", "third"]) 
emmision = scrapemmision("gene.counts")
threegram = scrape3gram("gene.counts")

tags = ["GENE","NOGENE","*","STOP"]
numtags = len(tags)

filetestname = "gene.dev"
fo = open("gene.dev.p2.out", 'w')
tokens = []
