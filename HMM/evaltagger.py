...
class Evaluator(object):
    ...
    def compare(self, gold_standard, prediction):
        ...
        for gs_word, gs_type in gold_standard: # Move through the gold standard stream
            ...
            pred_ends = curr_pred_type!=None and ((pred_type==None or pred_type == "NOGENE") or (curr_pred_type!=pred_type and pred_type=="GENE"))
            gs_ends = curr_gs_type!=None and ((gs_type==None or gs_type == "NOGENE") or (curr_gs_type!=gs_type and gs_type=="GENE"))
            ...
            if pred_word!=None:
                pred_start = (curr_pred_type==None and pred_type=="GENE") or (curr_pred_type!=None and curr_pred_type!=pred_type and pred_type=="GENE")
                gs_starts = (curr_gs_type==None and gs_type=="GENE") or (curr_gs_type!=None and curr_gs_type!=gs_type and gs_type=="GENE")
    ...
    def print_scores(self):
        ...
        for c in self.ne_classes:
            ...
            if (c_tp + c_fp) == 0:
                sys.stderr.write("Warning: prediction file does not contain any instances of entity type {}.\n".format(c))
                c_prec =1
            else:
